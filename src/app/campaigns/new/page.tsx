...
export default function NewCampaignPage() {
          ...
          if (newAddress) {
             ...
             const { error } = await supabase
               ...
               .insert({
                 ...
                 // 3. Save REAL Image URL to Database
  ...
  }, [isConfirmed, receipt, formData, creatorWallet, imageHash]);

  // Handlers
  const addMilestone = () => setMilestones([...milestones, { title: '', percent: 0, description: '' }]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 4. Validate Image
    if (!imageHash) {
        alert("Please upload a campaign banner image first.");
        return;
    }
      ...
      <div className="min-h-screen bg-[#0B0F14] text-slate-300 font-sans">
        ...
        <main className="max-w-3xl mx-auto px-6 pt-40 pb-24 text-center">
          ...
          <p className="text-lg text-slate-400 mb-8 max-w-xl mx-auto">
            Your campaign is live. It is now indexed in our database with your custom image.
          </p>
          ...
          <div className="bg-[#131823] border border-white/10 rounded-xl p-6 mb-10 max-w-lg mx-auto text-left">
            <div className="flex justify-between items-center mb-4">
              ...
              {deployedAddress ? (
                 ...
                 <span className="text-slate-600 italic text-sm animate-pulse">Syncing database...</span>
              )}
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-slate-500">Transaction</span>
              <a href={`https://sepolia.etherscan.io/tx/${hash}`} target="_blank" rel="noreferrer" className="font-mono text-teal-400 hover:underline text-sm truncate w-32 md:w-auto">
          ...
          <div className="flex gap-4 justify-center">
             <Link href="/campaigns" className="px-6 py-3 border border-white/10 text-white font-bold rounded-lg hover:bg-white/5 transition-colors">
               Dashboard
             </Link>
             {deployedAddress && (
               <Link href={`/campaigns/${deployedAddress}`} className="px-8 py-3 bg-teal-500 text-black font-bold rounded-lg hover:bg-teal-400 transition-colors shadow-lg shadow-teal-500/20">
                 View Campaign Page
               </Link>
             )}
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-24">
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <section className="space-y-6">
            ...
            {/* 6. NEW IMAGE UPLOAD SECTION */}
            <div className="mb-8">
                <label className="text-sm font-medium text-slate-400 mb-2 block">Campaign Banner</label>
                <IpfsUpload onUploadComplete={setImageHash} />
            </div>

            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-400">Campaign Title</label>
                <input type="text" required placeholder="e.g. Ocean Cleanup Phase 4" className="w-full bg-[#131823] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500/50 transition-colors" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-400">Category</label>
                <select className="w-full bg-[#131823] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500/50 transition-colors" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
            ...
            <div className="space-y-2">
              ...
              <textarea required rows={5} placeholder="Describe your project, goals, and impact..." className="w-full bg-[#131823] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500/50 transition-colors" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
            </div>

            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-400">Target Amount (ETH)</label>
                <input type="number" step="0.001" required placeholder="0.00" className="w-full bg-[#131823] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500/50 transition-colors font-mono" value={formData.target} onChange={e => setFormData({...formData, target: e.target.value})} />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-400">Duration (Days)</label>
                <input type="number" required className="w-full bg-[#131823] border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-teal-500/50 transition-colors" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} />
              </div>
            </div>
          </section>

          <div className="pt-8 border-t border-white/10">
            <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-teal-500 text-black font-bold text-lg rounded-xl hover:bg-teal-400 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
              {isWalletTriggered ? <><Loader2 className="animate-spin w-5 h-5" /> Check Wallet...</> : isConfirming ? <><Loader2 className="animate-spin w-5 h-5" /> Deploying...</> : <>Launch Campaign <ChevronRight className="w-5 h-5" /></>}
            </button>
          </div>
        </form>
      </main>
